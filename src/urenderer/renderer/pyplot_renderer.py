...
class PyplotRenderer(Renderer):
    ...
    def _stage_clipping(self, triangle: np.ndarray) -> tuple[bool, np.ndarray]:
        ...
        # Checa se o triângulo está dentro da cena
        clip = True
        for i in range(3):
            # x, y, z e w de cada vértice do triângulo
            x = triangle[i, 0]
            y = triangle[i, 1]
            z = triangle[i, 2]
            w = triangle[i, 3]
    ...
    def _stage_screen_mapping(self, triangle: np.ndarray) -> np.ndarray:
        ...
        triangle[:, :] = ((triangle[:, :] + 1)/2)   # Corrige as coordenadas para variar de 0 a 1
        triangle[:, 0] *= self.screen_width     # Multiplica o x pela largura do monitor
        triangle[:, 1] *= self.screen_height    # Multiplica o y pela altura do monitor
        #########################################################################
